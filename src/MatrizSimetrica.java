...
public class MatrizSimetrica {
    ...
    public static void main(String[] args) throws Exception {
        if (stdinput.hasNextInt()) {
            ...
            if(size > 0){
                ...
                rellenarMatriz(matriz,matrizT, 0, 0);
                stdinput.close();
    ...
    public static void rellenarMatriz(int[][] matriz, int[][] matrizT, int x, int y){
        if(x < matriz.length){
            String filaInput = stdinput.nextLine();
            if (!filaInput.trim().isEmpty()) {
                String[] fila = filaInput.split(" ");
                assert fila.length == matriz[y].length;
                rellenarFila(matriz, matrizT, x, 0, fila);
                rellenarMatriz(matriz, matrizT, x+1, 0);
            }
        }
    } 
    
    public static void rellenarFila(int[][] matriz, int[][] matrizT, int x, int y, String[] fila){
        if(y < matriz[x].length){
            matrizT[y][x] = matriz[x][y] = (int) Math.sqrt(Integer.parseInt(fila[y]));
            rellenarFila(matriz, matrizT, x, y+1, fila);
        }
    }
